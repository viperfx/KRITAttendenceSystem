app
		.controller(
				...
				function($scope, $compile, $state, $mdDialog, $mdMedia,
					...
					$scope.select = function(product, products) {
						var index = null;
						for (i in $scope.selected) {
							if (product == $scope.selected[i]) {
								index = i;
							}
						}
						if (index) {
					...
					$scope.selectAll = function(products) {
						 ...
						 */for (j in products) {
							var index = null;
							for (i in $scope.selected) {
								if (products[j].PRODUCT_ID == $scope.selected[i]) {
									index = i;
								}

							}
							if (index == null) {
								$scope.selected.push(products[j].PRODUCT_ID);
								selectedAll = true;
							}
					...
					$scope.selectedCheck = function(product) {
						for (i in $scope.selected) {
							if (product == $scope.selected[i]) {
								return true;
							}
					...
					$scope.selectedAllCheck = function(products) {
						for (j in products) {
							var index = null;
							for (i in $scope.selected) {
								if (products[j].PRODUCT_ID == $scope.selected[i]) {
					...
					ProductsService.categories(function(response, status) {
						...
						for (i in response) {
							...
							$scope.productCategory
									.addCategory($scope.productCategory
											.toJsonCategory());
						}

						$scope.states = $scope.productCategory
								.getAutoCompleteCategory();
						var tempCategories = $scope.productCategory
					...
					$scope.findProducts = function(selectedCategory) {
						...
						$scope.storedProducts = $scope.product.getProducts();
						var tempProducts = [];
						for (i in $scope.storedProducts) {
							for (j in $scope.selectedCategories) {

								if ($scope.storedProducts[i].PRODUCT_CATEGORY.PRODUCTCATEGORYID == $scope.selectedCategories[j].productCategoryId) {
									tempProducts.push($scope.storedProducts[i]);
									break;
								}
							}
						}
					...
					$scope.categorySearchRecursion = function(selectedCategory) {

						for (i in $scope.states) {
							if ($scope.states[i].productCategorySubcategory == selectedCategory.productCategoryId) {
								$scope
